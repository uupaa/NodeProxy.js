...
//{@dev
    $valid($type(url,      "String") && !URI.parse(url).error, NodeProxy_get, "url");
...
//{@dev
    ...
    $valid($type(url,    "String") && !URI.parse(url).error, NodeProxy_open, "url");
...
//}@dev
    ...
    var options = {
            host:   uri.hostname,   // without port number, "example.com:80" -> Error
            port:   uri.port || 80,
            path:   uri.path,

...
//{@assert
    _if(!Valid.type(name, "String"), NodeProxy.prototype.getResponseHeader, "name");
//}@assert
...
function NodeProxy_open(method,     // @arg String: "GET" or "POST", ...
                        ...
                        password) { // @arg String(= ""):
                                    // @help: NodeProxy#open
//{@assert
    _if(this._xhr.readyState !== READY_STATE_UNSENT,    NodeProxy.prototype.open, "sequence error");
    _if(!Valid.type(method, "String")  || !/^(GET|POST)$/.test(method),
                                                        NodeProxy.prototype.open, "method");
    _if(!Valid.type(url,    "String")  || URI.parse(url).error,
                                                        NodeProxy.prototype.open, "url");
    _if(!Valid.type(async,  "Boolean/omit"),            NodeProxy.prototype.open, "async");
    _if(!Valid.type(user,   "String/omit"),             NodeProxy.prototype.open, "user");
    _if(!Valid.type(password, "String/omit"),           NodeProxy.prototype.open, "password");
//}@assert
...
//{@assert
    _if(!Valid.type(mimeType, "String"), NodeProxy.prototype.overrideMimeType, "mimeType");
//}@assert
...
function NodeProxy_send(data) { // @arg Any(= null): POST request body
                                // @help: NodeProxy#send
//{@assert
    _if(this._xhr.readyState !== READY_STATE_OPENED, NodeProxy.prototype.send, "sequence error");
    _if(!Valid.type(data, "undefined"),              NodeProxy.prototype.send, "data");
//}@assert
...
//{@assert
    _if(!Valid.type(name, "String"),  NodeProxy.prototype.setRequestHeader, "name");
    _if(!Valid.type(value, "String"), NodeProxy.prototype.setRequestHeader, "value");
//}@assert
...
//}@node
if (global["NodeProxy"]) {
    global["NodeProxy_"] = NodeProxy; // secondary
} else {
    global["NodeProxy"]  = NodeProxy; // primary
}

...
//{@dev
    ...
    Valid(Valid.type(method, "String") && /^(GET|POST)$/.test(method),
                                                         NodeProxy_open, "method");
    Valid(Valid.type(url,    "String") && !URI.parse(url).error,
                                                         NodeProxy_open, "url");
...
function _createResponseValue(xhr) {
    ...
    case "arraybuffer":
    case "blob":
        result = DataType["Array"]["fromString"](text);
        break;
  //case "document":
    case "json":
